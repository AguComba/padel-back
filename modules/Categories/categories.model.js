...
export class CategoryModel {
  ...
  static async handleStatus(category) {
    try {
      const result = await executeQuery('UPDATE categories SET status = ?, user_updated = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [category.status, category.user_updated, category.id])
