...
const isValidPlayer = async (player, id_tournament, gender) => {
  const aceptedTournament = await TournamentModel.searchTournamentByCategoryPlayer(player.id_cat, id_tournament, gender)
  ...
  try {
    ...
    if (statusInscriptionPlayer) {
      return res.status(400).json({
        message: `Usted ya es encuentra inscripto con ${statusInscriptionPlayer.id_titular == player.id ? statusInscriptionPlayer.companero : statusInscriptionPlayer.titular}`,
      })
    }
    ...
    const categoryTournamentPlayer1 = await TournamentModel.searchTournamentByCategoryPlayer(player.id_cat, inscription.id_tournament, user.gender)
    ...
    if (statusInscriptionPlayer2) {
      return res.status(400).json({
        message: `Su compañero ya se encuentra inscripto con ${statusInscriptionPlayer2.id_titular == player2.id ? statusInscriptionPlayer2.companero : statusInscriptionPlayer2.titular}`,
      })
    }
    const categoryTournamentPlayer2 = await TournamentModel.searchTournamentByCategoryPlayer(player2.id_cat, validInscription.data.id_tournament)
