...
export class PlayerModel {
  ...
  static async create(player) {
    try {
      const [rows] = await connection.query(`INSERT INTO players (id_user, id_category, possition, hand, afiliation, id_club) VALUES (?, ?, ?, ?, ?, ?)`, [
        player.id_user,
        player.id_category,
        player.possition,
        player.hand,
      ...
      const [newPlayer] = await connection.query('SELECT * FROM players WHERE id = ?', [rows.insertId])
