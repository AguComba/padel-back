...
const runLocationsSeeder = async (filePath) => {
  ...
  try {
    ...
    for (const location of data.locations) {
      ...
      const sql = 'INSERT INTO cities (name, zip_code, id_province, status, created_at) VALUES (?, ?, ?, ?, NOW())'
...
const runUsersSeeder = async (filePath) => {
  ...
  try {
    ...
    for (const user of data.users) {
      const { name, last_name, cell_phone, email, type_document, number_document, gender, id_city, password, status, type_user } = user
      // Hasheo de la contraseña
      const hashedPassword = await bcrypt.hash(password, 10)
      const sql = 'INSERT INTO users (name, last_name, cell_phone, email, type_document, number_document, gender, id_city, password, status, type_user) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
      const values = [name, last_name, cell_phone, email, type_document, number_document, gender, id_city, hashedPassword, status, type_user]
      await connection.query(sql, values)
